...
if (isset($_POST["id"])) {
    ...
    if (empty($fname)) {
        ...
        echo "Please Select a Batch";
         ...
         $modelhasbrandId = Database::search("SELECT * FROM `student` WHERE `s_id`='" . $pid . "' ");
         $mhbi = $modelhasbrandId->fetch_assoc();

        Database::iud("UPDATE `student` SET `s_fname`='" . $fname . "', `s_lname`='".$lname."',`s_email`='".$email."',`s_nic`='".$nic."',`s_mobile`='".$mobile."',
        `s_password`='".$password."',`gender_id`='".$gender."',`batch_id`='".$batch."' WHERE `s_id`='".$pid."'");
