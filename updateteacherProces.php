...
if (isset($_POST["id"])) {
    ...
    if (empty($fname)) {
        ...
        echo "Please Select a subject";
         ...
         $modelhasbrandId = Database::search("SELECT * FROM `teacher` WHERE `t_id`='" . $pid . "' ");
         $mhbi = $modelhasbrandId->fetch_assoc();

        Database::iud("UPDATE `teacher` SET `t_fname`='" . $fname . "', `t_lname`='".$lname."',`t_email`='".$email."',`t_code`='".$tcode."',`t_mobile`='".$mobile."',
        `t_password`='".$password."',`gender_g_id`='".$gender."',`subject_id`='".$subject."',`reg_date`='".$date."' WHERE `t_id`='".$pid."'");
